...
inner join roles_usuarios r on u.id_rol_usuario_fk = r.id_rol_usuario
inner join casinos c on r.id_casino_fk = c.id_casino
where c.estatus_casino = 1 and r.estatus_rol = 1 and u.estatus_usuario = 1 and  u.email_usuario ='{$_POST["nick"]}' and u.contra_usuario = '{$_POST["contra"]}'");
...
if ($user->NumeroFilas() == 1){
    ...
    setcookie("CyTechnologies", base64_encode($json), time() + 28800, "/");
}
else{
